...
<?php
                ...
                echo"document.getElementById(\"delete$id\").onclick = function(){
                    ...
                    document.getElementById(\"update$id\").onclick = function(){
                        ...
                        if(str==\"\"){
                            ...
                            httpCon(\"php/updateBrand.php\",data,\"$method\",success2);
                        }
    ...
    function drawHTML($down,$cnt){
        echo "<div class='tab'>";
        $sql="select * from brand ORDER BY id DESC limit $down,$cnt";
        $con= consql();
        $arr=array();
        if($con){
        ...
        echo "<td>编号</td><td>品牌名称</td><td>删除</td><td>修改</td></tr>";
        foreach($arr as $row){
            echo "<tr>";
            $kid=$row[0];
            $kname=$row[1];
            echo "<td>$kid</td><td>$kname</td>";
            echo "<td><button id='delete$kid'><img src='img\icons\basic\delete.png' height='30' width='30'></button> </td>
                <td><input id='text$kid'></input><button id='update$kid'><img src='img\icons\basic\save.png' height='30' width='30'></button></td>
                </tr>";
        }
    ...
    echo "
        ...
        document.getElementById('pre').onclick=function(){
            if($pre>=0){
                window.location.href='brand.php?page=$pre';
            }
        }
        document.getElementById('next').onclick=function(){
            if($rowCnt==$maxCnt+1){
                window.location.href='brand.php?page=$next';
            }
        }
        </script>
    ";
